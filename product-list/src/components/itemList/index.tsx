...
const ItemList = () => {
  ...
  const deleteItem = (item: CartItem) => {
    const updatedList = listOfProducts().filter(
  ...
  const renderList = () => {
    ...
    return (
      <ul className="w-full h-[310px] flex flex-col">
        {productsList.length ? (
          productsList.map((item, index) => (
            <li
              key={index}
              className="border-solid border-b-2 border-color-[#C6C2C2] p-2"
            >
              <div className="flex flex-col sm:!flex-row justify-between items-center h-full items-center  ">
                ...
                <div className=" flex justify-end flex-col items-center gap-4 md:!flex-row">
                  <p className="font-semibold text-center">Qtd: {item.qtd}</p>
          ...
          <li className="flex flex-col items-center justify-center h-full gap-[10px]">
            <p className="text-xl">Não há itens na lista</p>
            <BiCartAdd size="3rem" />
          </li>
        )}
      </ul>
